...
pub fn solve() -> Result<(), io::Error> {
	...
	let claims = data.iter().fold(HashMap::new(), |mut claims, claim| {
		let r: Vec<usize> = claim.split(|c| c == ' ' || c == '@' || c == ',' || c == ':' || c == 'x' || c == '#')
		...
		for i in r[1]..r[1] + r[3] {
			for j in r[2]..r[2] + r[4] {
				all.insert(r[0]);
				claims.entry((i, j)).or_insert_with(HashSet::new).insert(r[0]);
			}
		}
		claims
	});
	...
	let out2 = all.difference(&overlaps).next().expect("Non overlapping claim not found");
